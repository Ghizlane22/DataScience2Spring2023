...
#the purpose of f2 is to check if x can be devided by y while the remainder is zero (x is a divisor of y)
### 3.2
...
my.mean(ex.vector)
mean(ex.vector)
#yes the two functions produce the same result that is (10)
...
rolldice1<-function(x){
  dice<-sample(1:6,x, replace=TRUE)
  return(dice)
}
rolldice1(5)

rolldice2<-function(x){
  n<-rolldice1(x)
  m<-rolldice1(x)
  print(n+m)
}
...
#double dice rolls(10 times)

data1<-replicate(2,rolldice2(10))
hist(data1, breaks =1:12)

#double dice rolls(50 times)

data2<-replicate(2,rolldice2(50))
hist(data2, breaks =1:12)

#double dice rolls(100 times)

data3<-replicate(2,rolldice2(100))
hist(data3, breaks =1:12)

#double dice rolls(1000 times)

data4<-replicate(2,rolldice2(1000))
hist(data4, breaks =1:12)

#double dice rolls(10000 times)

data5<-replicate(2,rolldice2(10000))
hist(data5, breaks =1:12)

#from all the histograms we can see that as the number of times we roll the dices increases as the distribution of results tends to be a normal distribution (more symmetric)
...
#The probability density in this case will be equal to 100%, because it's associated with all values of x between 0 and 1, Which means (1/(1-0)) that is equal to 1.
...
#The probability density in this case will also be equal to 1.because (1/ (6-5)) which is equal to 1.
...
#The probability density in this case will be equal to 2 .because (1/ (0.5-0)) which is equal to 2.
...
#The probability density in this case will also be equal to 0.5 .because (1/(2-0)) which is equal to 0.5(50%).
...
#This function is giving the probability density between the min and max values
